...
void delete_case5(node* x) {
    node* s = sibling(x);
    if (s->color == BLACK) {
        if ((x == x->parent->left) && (node_color(s->right) == BLACK) && 
            (node_color(s->left) == RED)) {
            s->color = RED;
            s->left->color = BLACK;
            right_rotate(s);
        }
        else if ((x == x->parent->right) && (node_color(s->left) == BLACK) && 
...
void print_tree_structure(node* curr, string indent, bool last) {
	...
	string node_color = (curr->color==0 ? "RED" : "BLACK");
    cout << curr->data << " (" << node_color << ")" << endl;
